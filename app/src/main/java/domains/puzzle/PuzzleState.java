...
//        return tilesOutOfPlace(this, goal);
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        ...
        for(int i = 1; i < 9; ++i) {
        Location cTile = currentState.getLocation(i);
        Location gTile = goalState.getLocation(i);
            if(!cTile.equals(gTile))
                ++count;
        }
    ...
    public static int sumManhattan(State current, State goal) {
        ...
        Location gTile = goalState.getLocation(i);
        int distance;
        int rows = abs(gTile.getRow() - cTile.getRow());
        int cols = abs(gTile.getColumn() - cTile.getColumn());
        distance = rows + cols;
        sum += distance;
